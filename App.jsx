...
function App() {
  ...
  useEffect(() => {
    if (selectedMember) {
      updateMetaTags({
        title: `${selectedMember.name} - ${selectedMember.specialty || ""} ${selectedMember.city ? "في " + selectedMember.city : ""}`.trim(),
        ...
        path: buildMemberPath(selectedMember),
  ...
  const navigate = (tab, filters={}) => { setActiveTab(tab); setSearchFilters(filters); };
